...
The project will investigate whether the computationally intensive environment light probing can be avoided while still achieving comparable levels of immersion in augmented reality scenes.
...
While there have been user-based studies evaluating visual characteristics of AR objects, this aspect appears not to have been examined yet.
...
Avoiding ACK implosion is essential to enable multicast file distribution to scale to large numbers of receivers. What techniques can we use and how should they be used.
...
Avoiding ACK implosion can be performed by sending only NACK, rather than sending positive ACK.
...
An analytic model showing the advantage of using only NACK versus positive ACK will be derived based on fitting curves to the experimental data for both forms of multicast file distribution. This model can be used to invalidate the hypothesis.
...
The project will use the empirical method, as it is not clear to the authors that we can use an analytical method to produce an accurate model of the protocol in the available time period for this assignment (given the available resources).
