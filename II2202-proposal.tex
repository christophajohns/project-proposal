...
While there has been extensive research on the effect of design decisions on several aspects of user experience in digital games~\cite{johnson_validation_2018} -- even in \gls{AR} games in a few cases (e.g.~\cite{georgiou_development_2017}) -- the effect of lighting and more specifically automatic naturalistic lighting in \gls{AR} games on immersion seems not to have been examined yet.
...
The main metrics we will collect for each of the two conditions, enabled and disabled automatic naturalistic lighting, are immersion level as measured through the \gls{IEQ} at the end of each run and \gls{CPU} usage over time as measured through Xcode's Developer Tools.
...
That is why we expect the lighting condition that more closely matches the scene's natural environment to lead to higher levels of immersion:
...
We instead hypothesize that users recognize the difference between both conditions and are able to articulate how they differ without support from the examiner.
...
Finally, regarding the energy impact of naturalistic lighting, we expect the computationally intensive continuous probing and estimation of the \gls{AR} scenes environment lighting to lead to higher levels of \gls{CPU} usage and thereby negatively affect battery life:
...
The implementation will be done for iOS devices and developed on Apple's \gls{IDE} Xcode with the ARKit and RealityKit frameworks.
...
Finally, we will measure subjects' game immersion and overall satisfaction in each task using the \gls{IEQ} proposed by Jennett et al.~\cite{jennett_measuring_2008}.
...
Participants will be recruited via an opportunity sample and will, therefore, mostly be international students at a Swedish university between 20 and 30 years old.
...
Due to the limited time available for recruitment, the expected number of participants in the small-scale study of this project is ten.
...
Following Jennett et al.~\cite{jennett_measuring_2008}, participants will be told to complete the card matching game as fast as possible and with as few errors as possible, before they are interrupted after a three minutes.
This should allow for a more challenging experience across player ability levels and, therefore, increase flow~\cite{csikszentmihalyi_flow_1990} and thereby general immersion levels as well as reduce overall variance in immersion~\cite{jennett_measuring_2008}.
